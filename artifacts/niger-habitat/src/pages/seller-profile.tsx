...
function ImageUpload({ label, value, onChange, className }: { label: string, value?: string, onChange: (url: string) => void, className?: string }) {  const [isUploading, setIsUploading] = useState(false);
  ...
  if (success) {
    return (
      <BaseModal isOpen={isOpen} onClose={onClose} title="Signalement envoyé">
         <div className="py-6 text-center flex flex-col items-center">            <div className="size-16 rounded-full bg-green-500/10 text-green-500 flex items-center justify-center mb-4"><CheckCircle size={32} /></div>
    ...
    <div className="pb-20">
      <div className="h-56 md:h-[340px] bg-muted relative">
        {shop.bannerUrl ? (
          ...
          <div className="w-full h-full ambient-grid bg-gradient-to-r from-primary/10 to-secondary/10" />            <div className="size-16 rounded-full bg-green-500/10 text-green-500 flex items-center justify-center mb-4"><CheckCircle size={32} /></div>
            <p className="text-foreground font-bold text-lg mb-2">Merci pour votre vigilance.</p>
            <p className="text-muted-foreground text-sm mb-6">Notre équipe de modération va examiner ce profil dans les plus brefs délais.</p>
            <button onClick={onClose} className="px-6 py-3 rounded-xl bg-foreground text-background font-bold w-full">Fermer</button>
         </div>
             ...
             <div className="space-y-6">
               ...
               <div className="space-y-2">
                 <label className="text-sm font-semibold text-foreground flex justify-between">
                   ...
                   <span className="text-xs text-muted-foreground">{formData.shopDescription.length}/700</span>Je continue avec les lignes 501 à 600 de `pages/seller-profile.tsx`.          )}
        </div>
      ) : (
...
function OwnerDashboard() {
  ...
  if (auth.isModerator) {
    tabs.push({ id: 'mod_requests', label: 'Modération: Vérif', icon: CheckCircle });
    tabs.push({ id: 'mod_reports', label: 'Modération: Signalements', icon: AlertTriangle });
  }

  return (
    <div className="page-shell py-12 md:py-16">
      <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-8">Espace Vendeur</h1>

      <div className="flex w-full overflow-x-auto hide-scrollbar mb-10">
        <div className="flex w-max min-w-full items-center gap-2 rounded-2xl bg-card p-2 border border-border shadow-sm">
          {tabs.map(tab => {
      ...
      {activeTab === 'profile' && <OwnerProfileEditor />}      {activeTab === 'verification' && <OwnerVerificationPanel />}
      {activeTab === 'mod_requests' && auth.isModerator && <ModVerificationRequests />}
      {activeTab === 'mod_reports' && auth.isModerator && <ModReports />}
    </div>
  );
}
