...
import { Send, Users, AlertCircle, Clock, RefreshCcw, Loader2 } from 'lucide-react';
import { usePaylocaAuth, authenticatedFetchForUser } from '@/auth/firebaseAuth';
import { useListFeedPosts, getListFeedPostsQueryKey } from '@workspace/api-client-react';
import { listQueuedFeedPosts, saveQueuedFeedPost, deleteQueuedFeedPost, type QueuedFeedPost } from '@/lib/offlineData';
import { createFeedQueueFlusher, processFeedQueueItems } from '@/lib/feedQueueControl';
...
export default function FeedPage() {
  ...
  const localQueue = storedLocalQueue.filter((item) => item.userId === activeUserId.current);

  useEffect(() => {
    if (user) {
      try {
        const saved = JSON.parse(localStorage.getItem(`payloca-feed-prefs-${user.id}`) ?? '{}');
  ...
  const saveProfile = () => {
    if (user && (community !== '' || city !== '')) {
      localStorage.setItem(`payloca-feed-prefs-${user.id}`, JSON.stringify({ community, city }));
      setNotice('Vos préférences sont enregistrées. Le fil mettra en avant votre communauté et ville.');
      setTimeout(() => setNotice(''), 3000);
    }
  };

  const { data: serverPosts, isLoading: postsLoading, isError: postsError } = useListFeedPosts();
  ...
  useEffect(() => { loadLocalQueue(); }, [loadLocalQueue]);
  ...
  const flushQueuePass = useCallback(async () => {
    ...
    const saveAndRefresh = async (item: QueuedFeedPost) => {
      ...
      if (activeUserId.current === ownerId) await loadLocalQueue();
    };
      ...
      send: async (item) => {
        try {
          ...
          if (response.status === 201) {
            ...
            await saveAndRefresh({ ...item, status: 'failed', lastError: errBody.error || 'Cette publication doit être corrigée avant un nouvel essai.' });
          } else {
            await saveAndRefresh({ ...item, status: 'queued', lastError: 'Serveur indisponible, réessai automatique' });
          }
        } catch {
          await saveAndRefresh({ ...item, status: 'queued', lastError: 'Hors ligne, en attente de connexion' });
        }
        if (activeUserId.current === ownerId) await loadLocalQueue();
      }
    });
  ...
  const flushQueue = useMemo(
    () => createFeedQueueFlusher(flushQueuePass, () => navigator.onLine),
    [flushQueuePass],
  );
  ...
  useEffect(() => {
    ...
    const onVisibility = () => { if (document.visibilityState === 'visible') flushQueue(); };
  ...
  const serverClientPostIds = new Set(serverPosts?.map(p => p.clientPostId) || []);
  const visibleQueue = localQueue.filter(q => !serverClientPostIds.has(q.input.clientPostId));
  ...
  const orderedServerPosts = useMemo(() => {
    ...
    const sameCommunityAndCity = sorted.filter(p => p.community === community && p.city === city);
    const sameCommunity = sorted.filter(p => p.community === community && p.city !== city);
    const sameCity = sorted.filter(p => p.city === city && p.community !== community);
    const rest = sorted.filter(p => p.community !== community && p.city !== city);
    ...
    <div className="page-shell max-w-3xl py-8 md:py-12">
      ...
      {!isSignedIn ? (
        <div className="mt-8 rounded-2xl border border-[#dfd7c4] bg-[#faf6ec] p-6 text-center shadow-sm">
          <p className="font-bold text-lg text-[#20283c]">Connectez-vous pour publier</p>
        ...
        <form onSubmit={publish} className="mt-8 rounded-2xl border border-[#dfd7c4] bg-[#faf6ec] p-5 shadow-sm transition-all focus-within:border-[#b95740] focus-within:shadow-[0_4px_16px_rgba(185,87,64,0.12)]">
          <div className="mb-4 grid gap-3 sm:grid-cols-2">
            <select aria-label="Communauté" value={community} onChange={(event) => setCommunity(event.target.value)} onBlur={saveProfile} className="rounded-xl border border-[#d9cfbc] bg-[#f4efdf] p-3 text-sm font-medium text-[#20283c] outline-none focus:border-[#b95740] focus:ring-1 focus:ring-[#b95740]">
              <option value="">Votre communauté (facultatif)</option>
              {communities.map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
            <select aria-label="Ville" value={city} onChange={(event) => setCity(event.target.value)} onBlur={saveProfile} className="rounded-xl border border-[#d9cfbc] bg-[#f4efdf] p-3 text-sm font-medium text-[#20283c] outline-none focus:border-[#b95740] focus:ring-1 focus:ring-[#b95740]">
              <option value="">Votre ville (facultatif)</option>
              {cities.map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <textarea aria-label="Votre publication" value={caption} onChange={(event) => setCaption(event.target.value)} maxLength={700} rows={3} placeholder="Partagez une idée ou un conseil avec votre communauté..." className="min-w-0 flex-1 resize-y rounded-xl border border-[#d9cfbc] bg-white p-3 text-sm font-medium text-[#20283c] outline-none focus:border-[#b95740] focus:ring-1 focus:ring-[#b95740]" />
            <button type="submit" disabled={!caption.trim()} className="inline-flex justify-center items-center gap-2 rounded-xl bg-[#b95740] px-6 py-3 text-sm font-bold text-white transition-transform active:scale-95 disabled:opacity-50 disabled:active:scale-100 shadow-sm">
      ...
      {notice && (
        <p role="status" className="mt-4 rounded-xl bg-[#eef7ed] p-3 text-sm font-semibold text-[#267158] border border-[#a3d9b1] animate-in slide-in-from-top-2">
          {notice}
        </p>
      )}
      ...
      <div className="mt-10 space-y-6">
        ...
        {!postsLoading && orderedServerPosts.length === 0 && visibleQueue.length === 0 && !postsError && (
          <div className="rounded-2xl border border-dashed border-[#d9cfbc] bg-transparent p-12 text-center text-[#596071]">
            <Users size={40} className="mx-auto text-[#d9cfbc] mb-4" />
            <p className="font-bold text-lg text-[#20283c]">Le fil est vide</p>
        ...
        {visibleQueue.map((item) => (
          <article key={item.id} className="overflow-hidden rounded-2xl border border-[#d9cfbc] bg-[#faf6ec]/60 shadow-sm transition-opacity">
            <div className="flex items-center justify-between border-b border-[#e7dfcf]/50 p-4 bg-[#f4efdf]/50">
              <div>
              ...
              {item.status === 'sending' && <span className="flex items-center gap-1.5 rounded-full bg-[#e8ddc6] px-3 py-1.5 text-xs font-bold text-[#596071]"><Loader2 size={12} className="animate-spin" /> Envoi...</span>}
              {item.status === 'queued' && <span className="flex items-center gap-1.5 rounded-full bg-[#f0e8d8] px-3 py-1.5 text-xs font-bold text-[#596071]"><Clock size={12} /> En attente de connexion</span>}
              {item.status === 'failed' && <span className="flex items-center gap-1.5 rounded-full bg-[#fff1eb] px-3 py-1.5 text-xs font-bold text-[#8f3e32]"><AlertCircle size={12} /> Non publié</span>}
            </div>
            <div className="bg-[#e8ddc6]/30 px-5 py-6 text-base font-medium leading-relaxed text-[#596071]">
              {item.input.caption}
            </div>
            {item.status === 'failed' && (
              <div className="flex items-center justify-between p-4 bg-[#fff1ec] border-t border-[#dca79b]/40">
                <span className="text-xs font-medium text-[#9d3526]">{item.lastError}</span>
                <button type="button" onClick={() => retryFailed(item.id)} className="text-xs font-bold text-[#b95740] hover:text-[#8f3e32] flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-[#f4dcd6] transition-colors"><RefreshCcw size={12} /> Réessayer</button>
              </div>
            )}
          </article>
        ))}
        ...
        {orderedServerPosts.map((post) => (
          <article key={post.id} className="overflow-hidden rounded-2xl border border-[#dfd7c4] bg-[#faf6ec] shadow-[0_4px_0_#e8deca]">
            ...
            <div className="bg-[#e8ddc6] px-5 py-8 text-lg font-semibold leading-relaxed text-[#20283c]">
              {post.caption}
            </div>
          </article>
        ))}
