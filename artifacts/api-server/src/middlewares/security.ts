...
function requestPath(req: Parameters<RequestHandler>[0]): string {
...
export function createRateLimiter(policy: RateLimitPolicy): RequestHandler {
  return (req, res, next) => {
    ...
    res.setHeader("X-RateLimit-Remaining", String(Math.max(0, policy.max - bucket.count)));
    res.setHeader("X-RateLimit-Reset", String(Math.ceil(bucket.resetAt / 1000)));

    if (bucket.count > policy.max) {
      res.setHeader("Retry-After", String(Math.max(1, Math.ceil((bucket.resetAt - now) / 1000))));
      res.status(429).json({ error: policy.message, code: "RATE_LIMITED" });
      return;
    }
    next();
  };
...
const sensitiveRateLimiter = createRateLimiter({
  windowMs: 10 * 60_000,
  max: 20,
  message: "Trop de tentatives sur cette action. Réessayez plus tard.",
});
...
export const protectApiRequests: RequestHandler = (req, res, next) => {
...
export const enforceBrowserOrigin: RequestHandler = (req, res, next) => {
  ...
  if (origin !== requestOrigin && !permitted.has(origin)) {
    res.status(403).json({ error: "Origine de requête non autorisée.", code: "ORIGIN_FORBIDDEN" });
    return;
  }
  next();
};
