...
export async function verifyFirebaseIdToken(
  ...
  try {
    return await getAuth(app).verifyIdToken(
      token,
    );
  } catch {
    return null;
...
export async function findFirebaseUserByPhoneNumber(
  phoneNumber: string,
): Promise<
  | {
      kind: "found";
...
> {
  ...
  try {
    const user =
      await getAuth(app).getUserByPhoneNumber(
        phoneNumber,
      );
    return {
      kind: "found",
...
export async function notifyPushRecipient(input: {
  ...
  const invalidTokens = devices
    .filter((_device, index) => {
      const errorCode =
        response.responses[index]?.error?.code;
      return (
        errorCode ===
