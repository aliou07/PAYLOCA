...
export async function getOrCreateReferralProfile(
  ...
  const [existing] =
    await db
      .select()
      .from(referralProfilesTable)
      .where(
        eq(
          referralProfilesTable.userId,
          userId,
        ),
      )
      .limit(1);
  ...
  throw new Error(
    "Impossible de créer votre code de parrainage.",
  );
}
...
export async function getReferralStats(
  ...
  const [countResult] =
    await db
      ...
      .where(
        eq(
          referralClaimsTable.referrerId,
          userId,
        ),
      );
  ...
  const shareUrl =
    `${
      ...
      encodeURIComponent(profile.code)
    }`;
  ...
  return {
    ...
    canClaim:
      !(await hasClaimedReferral(userId)),
  };
}

export async function hasClaimedReferral(
  userId: string,
  executor: any = db,
): Promise<boolean> {
  const [claim] =
    await executor
      ...
      .where(
        eq(
          referralClaimsTable.referredId,
          userId,
        ),
      )
      .limit(1);
...
export async function claimReferralCode(
  ...
  const claim =
    await db.transaction(async (tx) => {
      ...
      /*
       * Serialize claims for the same referrer so
       * concurrent filleuls cannot both observe
       * the same remaining quota.
       */
      await tx.execute(
        sql`
          select pg_advisory_xact_lock(
            hashtext(${referrer.userId})
          )
        `,
      );
