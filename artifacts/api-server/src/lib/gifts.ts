...
export function giftExpiryFrom(
  date = new Date(),
): Date {
...
export function giftMonthEndFrom(
  date: Date,
): Date {
...
export function createGiftPaymentPayload(input: {
  ...
  return {
    ...
    giftId: input.id,
    amount: giftPlans[input.plan].amount,
...
export function giftForResponse(
  gift: Gift,
...
export async function createPendingGift(input: {
  ...
  const [gift] = await db
    ...
    .values({
      ...
      amount: giftPlans[input.plan].amount,
      durationMonths:
        giftPlans[input.plan].durationMonths,
      code: createGiftCode(),
      transactionId,
  ...
  if (!gift) {
    throw new Error(
      "Impossible de créer le cadeau.",
    );
  }
  return gift;
}
export async function applyVerifiedGiftPayment(
  input: {
    giftId?: string;
    transactionId: string;
  ...
  const expectedAmount =
    giftPlans[input.plan].amount;
  if (input.amount !== expectedAmount) {
    throw new Error(
      "Montant du cadeau invalide.",
    );
  }
  return db.transaction(async (tx) => {
    ...
    if (gift.status !== "PENDING_PAYMENT") {
      throw new Error(
        "Le cadeau ne peut plus être payé.",
      );
    }
...
export async function automaticallyRedeemGiftIfPossible(
  gift: Gift,
): Promise<{
  gift: Gift;
  activated: boolean;
  ...
  const account =
    await findFirebaseUserByPhoneNumber(
      gift.toPhone,
    );
...
export async function redeemGift(input: {
  ...
  return db.transaction(async (tx) => {
    ...
    const status =
      gift.plan as "VIP_BRONZE" | "VIP_OR";
    const [updatedMembership] = await tx
      .update(membershipsTable)
      .set({
        status,
        trialEndsAt: giftMonthEndFrom(
    ...
    if (!updatedMembership) {
      throw new Error(
        "Impossible d’activer le forfait cadeau.",
      );
    }
    ...
    if (!redeemed) {
      throw new Error(
        "Ce code cadeau vient d’être utilisé.",
      );
    }
    const plan =
      planForMembershipStatus(status);
    const boostLimit =
      plan === "vip_or" ? 200 : 20;
    return {
      gift: redeemed,
...
export async function expireGiftIfNeeded(
  gift: Gift,
): Promise<Gift> {
  if (
    gift.status !== "PAID" ||
