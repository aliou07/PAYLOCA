...
export function giftForResponse(gift: Gift, direction: "sent" | "received") {
  const status: GiftStatus =
    gift.status === "PAID" && gift.expiresAt <= new Date()
      ? "EXPIRED"
...
export async function applyVerifiedGiftPayment(input: {
  ...
  return db.transaction(async (tx) => {
    const [gift] = await tx.select()
      .from(giftsTable)
      .where(eq(giftsTable.transactionId, input.transactionId))
      .limit(1);
    if (
      !gift
      || (input.giftId && gift.id !== input.giftId)
      || gift.plan !== input.plan
      || gift.amount !== input.amount
      || (
        input.durationMonths !== undefined
        && gift.durationMonths !== input.durationMonths
      )
    ) {
    ...
    if (
      ...
      || input.amount !== getPaidPlanOption(input.plan, durationMonths).amount
    ) {
      throw new Error("Montant du cadeau invalide.");
    }
    if (gift.status === "PAID" || gift.status === "REDEEMED") {
      return { applied: false, gift };
    }
    if (gift.status !== "PENDING_PAYMENT") {
      throw new Error("Le cadeau ne peut plus être payé.");
    }
    const [updated] = await tx.update(giftsTable)
      .set({
        status: "PAID",
      ...
      .where(and(
        eq(giftsTable.id, gift.id),
        eq(giftsTable.status, "PENDING_PAYMENT"),
      ))
      .returning();
    if (!updated) {
...
export async function redeemGift(input: {
  ...
  return db.transaction(async (tx) => {
    ...
    const [gift] = await tx.select()
      .from(giftsTable)
      .where(and(
        eq(giftsTable.code, input.code),
        eq(giftsTable.toPhone, input.phoneNumber),
      ))
      .limit(1);
    if (!gift) {
      throw new Error("Code cadeau ou numéro bénéficiaire invalide.");
    }
    if (gift.status === "PAID" && gift.expiresAt <= now) {
      await tx.update(giftsTable)
        .set({ status: "EXPIRED" })
        .where(eq(giftsTable.id, gift.id));
    ...
    if (gift.status !== "PAID") {
      throw new Error(
        gift.status === "REDEEMED"
          ? "Ce code cadeau a déjà été utilisé."
          : "Ce cadeau n’est pas encore disponible.",
      );
    }
    ...
    const durationMonths = isSubscriptionDuration(gift.durationMonths)
      ? gift.durationMonths
      : 1;
    ...
    if (!updatedMembership) {
      throw new Error("Impossible d’activer le forfait cadeau.");
    }
    const [redeemed] = await tx.update(giftsTable)
      .set({
        status: "REDEEMED",
        redeemedAt: now,
      ...
      .where(and(
        ...
        gt(giftsTable.expiresAt, now),
      ))
      .returning();
    if (!redeemed) {
      throw new Error("Ce code cadeau vient d’être utilisé.");
    }
...
export async function expireGiftIfNeeded(gift: Gift): Promise<Gift> {
  if (gift.status !== "PAID" || gift.expiresAt > new Date()) {
    return gift;
  }
  const [expired] = await db.update(giftsTable)
    .set({ status: "EXPIRED" })
    .where(and(
      eq(giftsTable.id, gift.id),
      eq(giftsTable.status, "PAID"),
      lte(giftsTable.expiresAt, new Date()),
    ))
    .returning();
  return expired ?? gift;
}
