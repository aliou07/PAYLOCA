...
export function createGiftPaymentPayload(
  input: {
    id: string;
...
export async function createPendingGift(
  ...
  const transactionId =
    `PAYLOCA_GIFT_${crypto.randomUUID()}`;
...
export async function automaticallyRedeemGiftIfPossible(
  ...
  try {
     ...
     * If a simultaneous claim won the race,
     * or activation is temporarily unavailable,
     * keep the paid gift claimable instead of
     * making Mynita retry a successful payment.
     */
    return {
      gift,
      activated: false,
