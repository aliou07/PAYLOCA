...
export class ObjectStorageService {
  ...
  normalizeObjectEntityPath(
    ...
    return `/objects/${url.pathname.slice(
      objectDir.length,
    )}`;
  }
  async getObjectEntityUploadURL(): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to sign upload URL (${response.status}).`,
      );
    }
    ...
    if (!data.signed_url) {
      throw new Error(
        "Storage returned no upload URL.",
      );
    }
  ...
  async getObjectEntityFile(
    ...
    const entityId = objectPath.slice(
      "/objects/".length,
    );
    if (!entityId || entityId.includes("..")) {
      throw new ObjectNotFoundError();
    }
    const objectDir = this.getPrivateObjectDir()
      .replace(/\/$/, "");
    const { bucketName, objectName } =
      parseObjectPath(
        `${objectDir}/${entityId}`,
  ...
  async verifyImage(
    ...
    return (
      ...
      (
        ...
        header.subarray(0, 4).toString() ===
          "RIFF" &&
        header.subarray(8, 12).toString() ===
          "WEBP"
      )
    );
  }
