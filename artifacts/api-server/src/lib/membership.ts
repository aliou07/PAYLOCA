...
export async function getActiveReferralWeeks(
  ...
  const [result] = await db
    .select({
      weeks: sql<number>`
        coalesce(
          sum(${referralRewardsTable.weeksAwarded}),
          0
        )::int
      `,
    })
    ...
    .where(
      and(
        eq(
          referralRewardsTable.userId,
          userId,
        ),
        gt(
          referralRewardsTable.expiresAt,
          now,
        ),
      ),
    );
...
export type SubscriptionDuration =
  typeof subscriptionDurations[number];
...
/**
 * Safety default: real money is impossible until
...
export function getPaidPlanOption(
  plan: PaidPlan,
  durationMonths: SubscriptionDuration = 1,
) {
  return paidPlans[plan].options[
    durationMonths
  ];
}

export function durationForPaidPlanAmount(
  plan: PaidPlan,
  amount: number,
): SubscriptionDuration | undefined {
  return subscriptionDurations.find(
    (duration) =>
      getPaidPlanOption(
        plan,
        duration,
...
export async function getOrCreateMembership(
  ...
  if (
    ...
    /*
     * Une autre requête peut avoir effectué
     * la transition entre la lecture initiale
     * et cet UPDATE. Dans ce cas, ne jamais
     * retourner l’essai obsolète.
     */
  ...
  const boostLimit =
    boostLimitForPlan(plan);

  const referralWeeksActive =
    await getActiveReferralWeeks(
      userId,
      now,
    );
...
/**
 * Les boosts sont un quota calendaire :
 * ils repartent le premier jour de chaque
 * mois UTC.
 */
