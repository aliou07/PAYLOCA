...
export async function canAccessYoungMedia(
  userId: string,
): Promise<boolean> {
  const [profile] = await db
    .select({
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      const uploadURL =
        await storage.getObjectEntityUploadURL();
      const objectPath =
        storage.normalizeObjectEntityPath(uploadURL);
      ...
      req.log.error(
        ...
        "Erreur lors de la génération de l’URL d’envoi de l’image",
      );
      res.status(500).json({
        error:
          "Le stockage des photos est momentanément indisponible.",
      });
    }
  },
);
router.post(
  "/storage/uploads/fun-video/request-url",
  requireAuth,
  async (req, res): Promise<void> => {
    if (
      !requireYoungExperience(
    ...
    const parsed =
      RequestFunVideoUploadUrlBody.safeParse(req.body);
    if (
      !parsed.success ||
      !allowedVideoTypes.has(parsed.data.contentType) ||
      parsed.data.size > maxVideoSize
    ) {
      res.status(400).json({
        error:
          "Vidéo invalide. Utilisez MP4, WebM ou QuickTime de 80 Mo maximum.",
      });
      return;
    }
    try {
      ...
      res.json(
        RequestFunVideoUploadUrlResponse.parse({
          uploadURL,
          objectPath,
          metadata: {
            name,
      ...
      req.log.error(
        ...
        "Erreur lors de la génération de l’URL d’envoi de la vidéo",
      );
      res.status(500).json({
        error:
          "Le stockage vidéo est momentanément indisponible.",
      });
    }
  },
);
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const [trackedImage] = await db
        ...
        .limit(1);
      const [trackedMedia] = await db
        .select()
        .from(storedMediaTable)
        .where(
          eq(storedMediaTable.objectPath, objectPath),
        )
        .limit(1);
      ...
      const token = authorization?.startsWith("Bearer ")
        ...
        : "";
      const userId = token
        ? (await verifyFirebaseIdToken(token))?.uid
        : null;
      let allowed = false;
      if (trackedMedia?.funVideoId) {
        ...
        if (!(await canAccessYoungMedia(userId))) {
          res.status(403).json({
            error:
              "Cet espace est réservé aux utilisateurs de moins de 30 ans.",
          });
          return;
        }
        const [video] = await db
          .select({
            id: funVideosTable.id,
          })
          .from(funVideosTable)
          .where(
            and(
              eq(
                funVideosTable.id,
                trackedMedia.funVideoId,
              ),
              eq(
                funVideosTable.moderationStatus,
                "published",
              ),
            ),
          )
        ...
        trackedImage &&
        (await isPublicSellerImage(
          objectPath,
          trackedImage.sellerProfileOwnerId,
        ))
      ) {
        allowed = true;
      ...
      req.log.error(
        ...
        "Erreur lors de la diffusion de l’image privée",
      );
      res.status(500).json({
        error: "La photo ne peut pas être affichée.",
      });
    }
  },
);
