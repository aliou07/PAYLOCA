      ...
      videos.map(async (video) => {
        const [{ value }] = await db
          ...
          .where(
            and(
              eq(
                funVideoCommentsTable.videoId,
                video.id,
              ),
              eq(
                funVideoCommentsTable.status,
                "published",
              ),
            ),
          );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      ...
      res.status(400).json({
        error:
          "La vidéo, la légende, la communauté et la ville doivent respecter les règles de sécurité PAYLOCA.",
      });
      return;
    }
    const existing = (
    ...
    if (
      ...
      res.status(400).json({
        error:
          "Cette vidéo n’est pas un fichier PAYLOCA FUN envoyé par votre compte.",
      });
      return;
    }
    try {
      ...
      if (
        ...
        res.status(400).json({
          error:
            "Les métadonnées de la vidéo ne correspondent pas au fichier envoyé.",
        });
        return;
      }
      ...
      if (!validHeader) {
        res.status(400).json({
          error:
            "Le fichier vidéo ne semble pas être un MP4, WebM ou QuickTime valide.",
        });
        return;
      }
    } catch (error) {
    ...
    if (!created) {
      res.status(409).json({
        error:
          "Cette vidéo est déjà en cours de publication.",
      });
      return;
    }
    res.status(201).json(
  ...
  async (req, res): Promise<void> => {
    ...
    const comments = await db
      .select({
        ...
        authorName:
          funVideoCommentsTable.authorName,
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      ...
      res.status(400).json({
        error:
          "Les numéros, e-mails et liens ne sont pas autorisés dans les commentaires.",
      });
      return;
    }
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const [created] = await db
      ...
      .onConflictDoNothing({
        target: [
          funVideoReportsTable.videoId,
          funVideoReportsTable.reporterId,
        ],
      })
      .returning();
    if (!created) {
