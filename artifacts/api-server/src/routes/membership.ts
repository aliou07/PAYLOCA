...
import { applySimulationPayment, applyVerifiedPayment, buildMynitaPaymentPayload, createPaymentReference, durationForPaidPlanAmount, getMynitaRedirectUrl, getOrCreateMembership, isPaidPlan, isSubscriptionDuration, paidPlans, PAYMENT_MODE, savePendingPayment } from "../lib/membership";
...
router.get("/membership/payments", requireAuth, async (req, res): Promise<void> => {
  try {
    const payments = await db.select({
      ...
      .where(eq(paymentEventsTable.userId, (req as AuthenticatedRequest).userId))
      .orderBy(desc(paymentEventsTable.processedAt));
    res.json(ListMembershipPaymentsResponse.parse(payments));
  } catch {
    res.status(503).json({ error: "L’historique des paiements est momentanément indisponible." });
  }
});
...
router.post("/membership/simulation", requireAuth, async (req, res): Promise<void> => {
  ...
  if ((plan !== "VIP_BRONZE" && plan !== "VIP_OR") || !isSubscriptionDuration(durationMonths)) {
    res.status(400).json({ error: "Forfait de simulation invalide." });
    return;
  }
  try {
    ...
    res.status(503).json({ error: "❌ Paiement échoué. Aucun argent n'a été débité." });
  }
});
...
router.post("/membership/payment", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!isPaidPlan(plan) || !isSubscriptionDuration(durationMonths)) {
    res.status(400).json({ error: "Forfait de paiement invalide." });
    return;
  }
  ...
  try {
    ...
    const providerResponse = await fetch(paymentUrl, {
      ...
      body: JSON.stringify(buildMynitaPaymentPayload({
        transactionId,
        userId: authenticated.userId,
    ...
    if (!providerResponse.ok || !redirectUrl) {
      ...
      req.log.warn({ statusCode: providerResponse.status, transactionId }, "Mynita payment request rejected");
      res.status(502).json({ error: "Mynita n’a pas pu préparer le paiement. Aucun droit n’a été modifié." });
      return;
    }
    res.status(201).json({ transactionId, plan, durationMonths, amount: paidPlans[plan].options[durationMonths].amount, redirectUrl });
  } catch (error) {
    await db.delete(paymentEventsTable).where(eq(paymentEventsTable.transactionId, transactionId)).catch(() => undefined);
    req.log.error({ err: error, transactionId }, "Mynita payment request failed");
    res.status(502).json({ error: "Impossible de joindre Mynita. Aucun droit n’a été modifié." });
  }
});
...
router.post("/membership/payment-return", async (req, res): Promise<void> => {
  ...
  const durationMonths = isPaidPlan(plan)
    ? (isSubscriptionDuration(body.durationMonths) ? body.durationMonths : durationForPaidPlanAmount(plan, amount))
    : undefined;
  const paymentStatus = typeof body.status === "string" ? body.status.toUpperCase() : "";
  const currency = typeof body.currency === "string" ? body.currency.toUpperCase() : "";
  if (!userId || !transactionId || !isPaidPlan(plan) || !durationMonths || !Number.isInteger(amount) || currency !== "XOF" || paymentStatus !== "SUCCEEDED") {
    res.status(400).json({ error: "Retour de paiement incomplet ou invalide." });
    return;
  }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Impossible de traiter le paiement.";
    res.status(message === "Montant du forfait invalide." ? 400 : 503).json({ error: message });
  }
});
