...
import { DeletePushTokenBody, RegisterPushTokenBody } from "@workspace/api-zod";
...
router.post("/push-tokens", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!parsed.success) {
    res.status(400).json({ error: "Jeton de notification invalide." });
    return;
  }
  const userId = (req as AuthenticatedRequest).userId;
  await db.insert(pushDevicesTable)
    .values({ userId, token: parsed.data.token, updatedAt: new Date() })
    .onConflictDoUpdate({
...
router.delete("/push-tokens", requireAuth, async (req, res): Promise<void> => {
  const userId = (req as AuthenticatedRequest).userId;
  if (!req.body?.token) {
    await db.delete(pushDevicesTable).where(eq(pushDevicesTable.userId, userId));
    res.status(204).send();
    return;
  }
  ...
  await db.delete(pushDevicesTable).where(and(
    ...
    eq(pushDevicesTable.token, parsed.data.token),
  ));
  res.status(204).send();
});
