...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    res.json(
      ListGiftsResponse.parse(result),
    );
  },
);
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const parsed =
      CreateGiftPaymentBody.safeParse(
        req.body,
      );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    if (!hasValidPaymentSignature(req)) {
      res.status(401).json({
        error:
          "Retour de paiement non authentifié.",
      });
      return;
    }
