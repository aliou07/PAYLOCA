...
router.get("/gifts", requireAuth, async (req, res): Promise<void> => {
  ...
  const sent = await db.select()
    .from(giftsTable)
    .where(eq(giftsTable.fromUserId, authenticated.userId));
  const received = authenticated.phoneNumber
    ? await db.select()
      .from(giftsTable)
      .where(eq(giftsTable.toPhone, authenticated.phoneNumber))
    : [];
  const result = [
    ...(await Promise.all(
      sent.map(async (gift) =>
...
router.post("/gifts/payment", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!toPhone) {
    res.status(400).json({
      error: "Indiquez un numéro nigérien valide au format +227 suivi de 8 chiffres.",
    });
    return;
  }
  ...
  if (!paymentUrl || !apiKey || !callbackUrl) {
    res.status(503).json({
      error: "Le paiement Mynita des cadeaux n’est pas configuré.",
    });
    return;
  }
  const authenticated = req as AuthenticatedRequest;
  ...
  try {
    ...
    if (!providerResponse.ok || !redirectUrl) {
      ...
      res.status(502).json({
        error: "Mynita n’a pas pu préparer le cadeau. Aucun argent n’a été débité.",
      });
      return;
    }
    ...
    res.status(502).json({
      error: "Impossible de joindre Mynita. Aucun cadeau n’a été créé.",
    });
  }
});
router.post("/gifts/payment-return", async (req, res): Promise<void> => {
  ...
  const giftId = typeof body.giftId === "string"
    ? body.giftId.trim()
    : undefined;
  ...
  const amount = typeof body.amount === "number"
    ? body.amount
    : Number(body.amount);
  const durationMonths = isSubscriptionDuration(body.durationMonths)
    ? body.durationMonths
    : undefined;
  ...
  if (
    ...
    res.status(400).json({
      error: "Retour de paiement cadeau incomplet ou invalide.",
    });
    return;
  }
  try {
    ...
    res
      .status(message === "Montant du cadeau invalide." ? 400 : 503)
      .json({ error: message });
  }
});
router.post("/gifts/redeem", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!authenticated.phoneNumber) {
    res.status(403).json({
      error: "Votre numéro Firebase vérifié est nécessaire pour utiliser un cadeau.",
    });
    return;
  }
  try {
    ...
    res
      .status(
        message.includes("déjà") || message.includes("expiré")
          ? 409
          : 400,
      )
      .json({ error: message });
